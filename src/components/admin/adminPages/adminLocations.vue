<template>
  <h2 class="data_title">Локации</h2>
  <br>
...
<div>
  <table class="admin_table">
    ...
    <tr class="record" @click="showDialog" v-for="item in locations" :key="item.id">
      <td>{{ item.id }}</td>
      <td>{{ item.name }}</td>
      <td>{{ item.address }}</td>
      <td>{{ item.type }}</td>
...
export default {
  ...
  data() {
    return{
      dialogVisible: false,
      locations: [
        { id: 1, name: 'Пентхаус', address: 'ул.Северная 120/1', type: 'Партнер', status: 'Принято', link: 'https/google.com', gps: '39.7136063', companyName: 'Оджетто' }
      ]
    }
  },
  methods: {
